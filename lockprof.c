...
init(void)
{
    ...
    /* Make sure that threads are spawned on the same socket */
    CPU_ZERO(&set);
    CPU_SET(thread_to_core_map[0], &set);
    CPU_SET(thread_to_core_map[1], &set);
    CPU_SET(thread_to_core_map[2], &set);
    CPU_SET(thread_to_core_map[3], &set);
